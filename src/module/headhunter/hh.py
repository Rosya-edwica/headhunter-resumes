...
def find_profession_in_currrent_city(profession: Position, city: City):
    ...
    for page in range(pages):
        ...
        scrape_professions_in_page(page=f"{url}&page={page}", positionId=profession.Id)


def scrape_professions_in_page(page: str, positionId: int) -> None:
...
def scrape_resume(block: BeautifulSoup, positionId: int = 1):
    ...
    resume_scraper = Scraper(resume_url, positionId, resume_date_update)
